...
try:
    ...
    # CDP wallet and X402 payment integration
    from cdp_wallet_x402_integration import handle_cdp_x402_request
...
def handle_combined_wallet_payment_request(event):
    ...
    try:
        ...
        if 'handle_cdp_x402_request' not in globals():
            from cdp_wallet_x402_integration import handle_cdp_x402_request
        
        # Forward the request to the CDP wallet and X402 payment handler
        return handle_cdp_x402_request(event)
    except Exception as e:
        logger.error(f"Error handling CDP wallet or X402 payment request: {str(e)}")
        return {
            'statusCode': 500,
